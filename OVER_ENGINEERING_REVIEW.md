...
- **Spec drift in the closed sets.** `Verdict` lives in `schemas/report.py:27`, not `interpret/tree.py` as `SKILL.md:143/187` claims, **and the values don't match** (code: `LEARN-UNDERPOWERED/NO-SHIP-PRIMARY/ITERATE-WEAK/ITERATE-NOVELTY`; SKILL: `INCONCLUSIVE/NO-LIFT/DIRECTIONAL-ONLY/LIFT-WITH-CAVEAT`). The skill that drives the agent describes a different system than the code implements.
