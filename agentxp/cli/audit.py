"""agentxp audit — the audit-trail CLI for an experiment (§15 / D4).

v0.1 ships 3 surfaces (Theme 7 cut, per D4 / M64):
  - ``agentxp audit <exp_id>``                       — text timeline (default)
  - ``agentxp audit <exp_id> --diff <other_exp_id>`` — pairwise diff
  - ``agentxp audit <exp_id> --html``                — self-contained HTML report
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    if args.html:
        if not exp_dir.exists():
            ...
            return EXIT_USER_ERROR
        try:
            from agentxp.cli.audit_html import render_html_report, write_html_report
        except Exception as e:
            print(f"unexpected error: {type(e).__name__}: {e}", file=sys.stderr)
            return EXIT_FATAL
