...
class SrmOverrideReasonCode(str, Enum):
    ...
    Per the spec, this enum's canonical home is ``agentxp.schemas.gate`` — but
    ...
    ``agentxp.schemas.gate``) can be retargeted to ``agentxp.schemas.state`` if
    needed for the W_pre1 unblock. ``agentxp.schemas.gate`` should re-export from
    here when it lands.
    """
