...
from agentxp.audit.storage import append_conversation_turn, append_event
...
def _default_system_prompt_path(project_root: Path, agent_name: str) -> Path:
    ...
    state.yaml lives), NOT the agentxp install root. The agent prompts ship
    ...
    walk up from this file: ``agentxp/orchestrator/dispatch.py`` →
    ``agentxp/`` → ``<repo>/agents/{name}.system.md``.
    """
    # __file__ → .../agentxp/orchestrator/dispatch.py
    # parents[0] = orchestrator/, parents[1] = agentxp/, parents[2] = repo root
...
def _invoke_llm(system_prompt: str, ctx_bundle: dict[str, Any]) -> str:
    ...
    Tests monkeypatch this symbol via
        monkeypatch.setattr("agentxp.orchestrator.dispatch._invoke_llm", fake)
    ...
    raise NotImplementedError(
        ...
        "agentxp.orchestrator.dispatch._invoke_llm in tests"
    )
