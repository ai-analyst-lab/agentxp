...
from agentxp.metrics.schema import MetricDefinition, MetricValidationError, validate


def _default_metrics_dir() -> Optional[Path]:
    """Return the first metrics directory that exists: ./metrics then ~/.agentxp/metrics."""
    cwd_metrics = Path.cwd() / "metrics"
    if cwd_metrics.is_dir():
        return cwd_metrics
    home_metrics = Path.home() / ".agentxp" / "metrics"
