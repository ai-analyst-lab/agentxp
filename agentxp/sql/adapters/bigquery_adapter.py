"""BigQuery warehouse adapter — STUB for AgentXP v0.1 (§12).

The full BigQuery adapter (ADC / service-account JSON auth, native
``maximumBytesBilled`` enforcement, dry-run cost estimates) lands in v0.1.1
per §12 / D1. v0.1 ships this stub so the BaseAdapter Protocol has a
registered implementation under the ``bigquery`` dialect.
...
class BigQueryAdapter:
    ...
    def execute(
        ...
        raise NotImplementedError("BigQuery adapter ships in v0.1.1")
