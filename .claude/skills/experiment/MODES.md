...
All stats calls use the full import path (e.g., `from openxp.stats.power import power_proportion`). All agent references use the repo-relative path (e.g., `agents/experiment-analyzer.md`).
...
2. **Invoke designer agent.** Read `agents/experiment-designer.md` and execute its 7-step conversation flow:
   ...
   - Step 4: Power calculation (via `openxp.stats.power.power_proportion` / `power_mean` / `power_ratio` depending on metric type).
   - Step 5: Duration estimate via `openxp.stats.power.duration_estimate` (rounds to 7-day multiple, D.27).
   - Step 6: Sensitivity table via `openxp.stats.power.power_sensitivity_table`.
...
- Optional: alpha (default 0.05), power (default 0.80), allocation (default 0.50).
...
2. **Dispatch to the correct power function** (no agent — this is direct stats):
   - `proportion` → `from openxp.stats.power import power_proportion; power_proportion(baseline, mde, alpha, power, mde_type='relative')`
   - `continuous` → `from openxp.stats.power import power_mean; power_mean(baseline_mean, baseline_std, mde, alpha, power)`
   - `ratio` → `from openxp.stats.power import power_ratio; power_ratio(num_mean, num_var, den_mean, den_var, cov, mde, alpha, power)`
3. **Duration estimate.** `from openxp.stats.power import duration_estimate; duration_estimate(result['total_sample_size'], daily_traffic, allocation)`. Returns `{days, viable, projected_end_date}`. Rounds to 7-day multiple (D.27).
4. **Sensitivity table.** `from openxp.stats.power import power_sensitivity_table; power_sensitivity_table(baseline, [0.03, 0.05, 0.10, 0.15], [daily_traffic, 2*daily_traffic])`. Save to `working/sensitivity-table.csv`.
...
- Ratio metric without covariance: warn user that assuming cov=0 is conservative and may overestimate duration.
...
2. **Data preparation.** Call `from openxp.stats.prep import prepare_experiment_data; prepare_experiment_data(df, variant_col, metric_col, unit_col, metric_type)`. Returns clean arrays + data quality report. If quality fails → **INVALID** with specific diagnosis (skip to interpret).
...
4. **SRM gate (Type C).** Call `from openxp.stats.srm import srm_check; srm_check(observed_counts, expected_ratios, threshold=0.0005)`.
   - `PASS` (p > 0.01): continue.
   - `WARNING` (0.0005 < p <= 0.01): surface warning, continue with caveat.
   - `BLOCK` (p <= 0.0005): **halt**. Run `srm_diagnose(assignments_df, segments)`. Write INVALID verdict to `analysis_results.json`. Transition to `interpret` mode which will finalize INVALID, or offer the SRM recovery path (`ANALYZING -> COLLECTING` if root cause is fixable).
5. **Invoke experiment-analyzer agent.** Read `agents/experiment-analyzer.md`. The agent walks the 8-question framework:
   ...
   - Q2: Primary metric test. Dispatch on metric type:
     - proportion → `from openxp.stats.ab_tests import proportion_test`. If expected cell count < 5, fall back to `fishers_exact_test`.
     - continuous → `from openxp.stats.ab_tests import welch_test`. Winsorize first if skewness > 3 (already done in prep).
     - ratio → `from openxp.stats.ab_tests import ratio_metric_test`. Also call `denominator_srm` (D.23).
   - Q3: Effect size. `cohens_d` (continuous), `cohens_h` (proportion), always `relative_lift`. MDE at observed n via `detectable_effect`.
...
6. **Guardrail tests.** For each guardrail metric: `from openxp.stats.ab_tests import guardrail_test; guardrail_test(control, treatment, nim=..., alternative='less')`. Uses non-inferiority test against NIM if set; otherwise simple inferiority test.
7. **Checkpoint: Guardrail violation (Type C).**
   - **Never skip.**
   - If any `guardrail_test` returns `FAIL`: surface with lift, CI, NIM breach magnitude, and ask user to acknowledge before proceeding.
8. **Multiple comparisons.** Secondary metrics only: `from openxp.stats.corrections import adjust_pvalues; adjust_pvalues(secondary_pvalues, method='holm')`. Primary never corrected. Guardrails never corrected.
...
2. **Invoke experiment-interpreter agent.** Read `agents/experiment-interpreter.md`. The agent walks PRD **Appendix A Result Interpretation Tree**:
   ...
   - **Branch 5b: LEARN (underpowered)** — Null primary + achieved MDE > planned MDE. Call `from openxp.stats.power import extension_estimate; extension_estimate(current_n, required_n, daily_traffic, allocation)` and report: additional days, additional observations, revised MDE at both current and extended n.
...
3. **Powered-as-spectrum reporting (D.26).** Never binary. Always report: "At N=X, you can detect effects >= Y%. Smaller effects cannot be ruled out." Use `detectable_effect` at observed n.
...
2. **Invoke experiment-monitor agent.** Read `agents/experiment-monitor.md`. The agent runs 4 checks, each producing a traffic-light status:
   - **Check 1: SRM trending.** `srm_check(overall_counts, threshold=0.0005)` + per-day `srm_check` if timestamps available. Three-tier (D.14): GREEN (p > 0.01), YELLOW (0.0005 < p <= 0.01), RED (p <= 0.0005).
   - **Check 2: Guardrail status.** For each guardrail: `from openxp.stats.ab_tests import guardrail_test; guardrail_test(control, treatment, nim, alternative='less')`. RED = FAIL, YELLOW = WARNING, GREEN = PASS. Also check `emergency_stop` rule from YAML (>15% relative degradation).
   ...
   - **Check 4: Detectable effect at current sample.** `from openxp.stats.power import detectable_effect; detectable_effect(n_per_group, baseline, ...)`. Shows what MDE is currently detectable vs planned MDE.
...
6. **Checkpoint: Guardrail RED / SRM RED (Type C).**
   ...
   - SRM RED → offer immediate `srm_diagnose` + SRM recovery path (ANALYZING -> COLLECTING with salt change if fixable).
...
2. **Agents never re-implement math.** If an agent is tempted to write `import scipy.stats` directly, stop it. Use `openxp.stats.*` imports only.
