...
# ──────────────────────────────────────────────────────────────────────────
# Shared UTC validator. Mirror of ``openxp.schemas.state._enforce_utc`` —
...
def _enforce_utc(v: datetime) -> datetime:
    ...
    in OpenXP are UTC ISO-8601 with a ``Z`` suffix; cohort-local windows are
...
class SourceType(str, Enum):
    ...
    credentials live in ``~/.openxp/credentials/{type}/{profile}.yaml`` per
    §1.7.3.
    """
