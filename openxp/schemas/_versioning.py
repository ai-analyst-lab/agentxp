"""Schema-version registry + load-refusal helpers for OpenXP v0.1.

Single source of truth for which ``schema_version`` of each persisted YAML/JSON
file is supported by this OpenXP version. Loading a file with
...
"""Max ``schema_version`` this OpenXP can load for each persisted file pattern.
...
"""Minimum ``schema_version`` this OpenXP accepts. v0.1 has no v0 grace period."""
...
class SchemaVersionTooNew(SchemaVersionError):
    """File was written by a newer OpenXP. Upgrade with: ``pip install --upgrade openxp``."""


class SchemaVersionTooOld(SchemaVersionError):
    """File was written by an older OpenXP. Run: ``openxp migrate state <exp_id>``."""
...
def check_schema_version(file_path: Path, raw_data: dict[str, Any]) -> int:
    ...
    if "schema_version" not in raw_data:
        raise SchemaVersionMissing(
            ...
            f"This file is unreadable by OpenXP v0.1. If you wrote it by hand, "
    ...
    if version > max_supported:
        raise SchemaVersionTooNew(
            f"File {file_path} has schema_version={version}, but this OpenXP "
            f"supports up to {max_supported} for {pattern!r}. "
            f"This file was written by a newer OpenXP. "
            f"Upgrade with: pip install --upgrade openxp"
        )
    ...
    if version < min_supported:
        raise SchemaVersionTooOld(
            ...
            f"This file was written by an older OpenXP. "
            f"Run: openxp migrate state <exp_id>"
        )
