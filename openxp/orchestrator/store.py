...
- The ``.state.lock`` sidecar holds a JSON ``LockMetadata`` envelope so
  ``openxp resume`` can detect a stale-lock PID per §10.6.3 / §1.8.12.
...
``openxp.schemas.state``); error messages crossing into the audit log
...
from openxp.audit.redactor import redact_message
from openxp.audit.storage import _atomic_write_bytes, append_event
...
class StaleLockError(Exception):
    ...
    The user runs ``openxp unlock <exp_id>`` (W5) to break the lock; this
...
class OrchestratorStore:
    """The single stage-commit chokepoint for OpenXP v0.1.
    ...
    def _file_lock(self, timeout_s: Optional[float] = None) -> Iterator[None]:
        ...
        can route to ``openxp unlock``. The sidecar JSON envelope is
                    ...
                    if not stale_checked:
                        ...
                        if meta is not None and not self._pid_is_alive(meta.pid):
                            ...
                            raise StaleLockError(
                                ...
                                "run 'openxp unlock <exp_id>' to break"
                            )
                        stale_checked = True
