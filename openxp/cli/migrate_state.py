"""CLI stub: ``openxp migrate state <exp_id>``.

In v0.1 this command is a no-op stub that prints a friendly message and exits
zero. It exists so the load-refusal contract in :mod:`openxp.schemas._versioning`
can point users at a real command (``SchemaVersionTooOld`` messages tell the
user to run ``openxp migrate state <exp_id>``).
...
from openxp.cli.exit_codes import EXIT_OK


def main(argv: list[str] | None = None) -> int:
    """Entry point for ``openxp migrate state <exp_id>``.

    Returns ``EXIT_OK`` (0) in v0.1 — there is nothing to migrate yet.
    """
    parser = argparse.ArgumentParser(
        prog="openxp migrate state",
        description=(
            "Migrate an experiment's persisted state files from a prior "
            "schema_version to the version supported by this OpenXP."
        ),
    )
    parser.add_argument(
        "experiment_id",
