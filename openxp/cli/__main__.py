"""CLI dispatcher for the openxp binary.

v0.1: only `openxp profile` is wired. The other six subcommands print a
"ships in <wave>" message and exit with EXIT_USER_ERROR. W5 / W_sql /
W_hooks fill in the registry rows as their CLIs land.

Entry point per pyproject.toml `[project.scripts] openxp = openxp.cli.__main__:main`.

Also callable via `python -m openxp`.
"""
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    if subcommand not in SUBCOMMANDS:
        print(f"openxp: unknown subcommand {subcommand!r}", file=sys.stderr)
    ...
    if isinstance(target, str):
        print(
            f"openxp {subcommand}: not yet implemented (ships in {target})",
            file=sys.stderr,
        )
        return EXIT_USER_ERROR
...
def _print_help(stream) -> None:
    print("usage: openxp <subcommand> [args]", file=stream)
    print("", file=stream)
    ...
    print("", file=stream)
    print("Run `openxp <subcommand> --help` for subcommand-specific options.", file=stream)
