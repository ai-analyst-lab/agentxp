"""openxp resume — recovery / resumption CLI for an experiment (W5.3).
...
def _detect_case(
    ...
    if isinstance(schema_version, int) and schema_version < 3:
        return (
            ...
            "Run `openxp migrate state` to upgrade.",
        )
    ...
    if isinstance(stage_history, list):
        ...
        if dup_stage is not None:
            return (
                ...
                "stage_history. Recommend `openxp resume <exp_id> --force` after "
                "auditing the chain (`openxp audit <exp_id>`).",
            )
