"""openxp profile — Stage-0 profiler CLI (W_pre2.4).

Wires the fast SUMMARIZE-based driver (W_pre2.1) and the optional
ydata-profiling deep sidecar (W_pre2.3) into a single ``openxp profile``
...
def _validate_path_under_allowed_roots(path: Path, *, arg_name: str) -> Path:
    """Resolve path and assert it's under cwd or ~/.openxp/.
    ...
    openxp_home = (Path.home() / ".openxp").resolve()
    for root in (cwd, openxp_home):
        try:
            ...
            continue
    raise ValueError(
        f"{arg_name} path must be under {cwd} or {openxp_home}; got {resolved}"
    )
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    # Path guards — write only under cwd or ~/.openxp/
    try:
    ...
    try:
        from openxp.profiler.driver import profile_dataset, write_profile_bundle
    ...
    if deep_requested:
        ...
        from openxp.profiler.ydata_sidecar import run_ydata_deep_profile
