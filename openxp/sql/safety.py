"""5-layer SQL safety pipeline for OpenXP v0.1 (§11).

Walks user / agent SQL through five gates before dispatch:

  1. parse           — sqlglot tree (delegated to :mod:`openxp.sql.parser`)
...
# bytes_scanned_cap, require_explain) lives in openxp.sql.schema; the
...
class SafetyResult(BaseModel):
    ...
    sql_validated: str
    tree: Any  # sqlglot AST (kept opaque outside openxp.sql)
...
def layer_3c_deny_list_check(tree: exp.Expression) -> None:
    ...
    * **Function deny-list:** every :class:`exp.Anonymous` / :class:`exp.Func`
      ...
      :data:`openxp.sql.parser.DENY_FUNCTIONS`.
    * **AST allowlist:** every visited node's ``type()`` must be in (or be a
      subclass of) :data:`openxp.sql.parser.ALLOWED_AST_NODES` or the
