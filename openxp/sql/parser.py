"""Layer-1 SQL parsing for the OpenXP safety pipeline (§11).

Wraps :mod:`sqlglot` so the rest of ``openxp.sql`` can treat the AST as
opaque. Only this module and :mod:`openxp.sql.transpiler` import sqlglot
...
performed by :func:`openxp.sql.safety.layer_3c_deny_list_check`.
"""
...
:class:`openxp.sql.safety.DenyListViolation`.
"""
...
def parse_sql(sql: str, dialect: str) -> exp.Expression:
    ...
    Raises :class:`openxp.sql.safety.UnparseableSQL` when sqlglot cannot
    ...
    from openxp.sql.safety import UnparseableSQL
