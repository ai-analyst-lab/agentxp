...
**Scope:** `openxp/data/`, `openxp/stats/{cuped,sequential,bayesian}.py`, `openxp/storage/`, `openxp/metrics/`, skill orchestrator, 183 new tests
...
- **Blocker before Wave 2: the `/experiment` orchestrator skill references ~7 functions and 1 module that do not exist** (`power_ratio`, `extension_estimate`, `fishers_exact_test`, `guardrail_test`, `denominator_srm`, `cohens_h`, `openxp.stats.prep.prepare_experiment_data`). It also advertises a `computation_trace` dict field that no Wave 1 function returns. This will break every mode except `power` and `status` the moment it runs.
...
The "Stats Function Quick Reference" table and per-mode step specs name these functions that do not exist in `openxp.stats`:

| Claimed in skill/MODES | Actually exists? |
|---|---|
| `openxp.stats.power.power_ratio` | No |
| `openxp.stats.power.extension_estimate` | No |
| `openxp.stats.ab_tests.fishers_exact_test` | No |
| `openxp.stats.ab_tests.guardrail_test` | No |
| `openxp.stats.srm.denominator_srm` | No |
| `openxp.stats.effect_size.cohens_h` | No |
| `openxp.stats.prep.prepare_experiment_data` (whole module) | No |
...
No Wave 1 function (or pre-Wave 1 function) returns a `computation_trace` key — I grepped `openxp/stats/` for it and got zero matches. MODES.md §`interpret` step 2 says "D.9 validation" on `computation_trace` entries, and §`analyze` step 10 says to write traces. If the orchestrator enforces this contract, the skill will reject every analysis it runs.

### C3. `openxp/__init__.py` advertises an API it doesn't expose — `openxp/__init__.py:11-32`
The top-level docstring shows `from openxp.stats import welch_test, proportion_test, ...`, but `__all__` is only `["load_data", "discover_schema"]` and the root package does not re-export `stats`, `storage`, or `metrics` as attributes. `import openxp; openxp.stats.welch_test` fails (you have to `import openxp.stats` explicitly). Not a blocker — the doc example still works verbatim — but a linter or `help(openxp)` user will be confused. Also: Wave 1's new Bayesian, CUPED, and sequential APIs are not mentioned in the top-level docstring usage examples at all.

---

## 3. Math / Correctness Issues (would give wrong answers)

### M1. `normal_normal_test` per-draw shrinkage is mathematically wrong for strong priors — `openxp/stats/bayesian.py:378-389`
```python
...
### M2. `_msprt_core` contains a dead algebraic remnant — `openxp/stats/sequential.py:104`
```python
...
### M3. CUPED math is correct, including the pooled-theta requirement — `openxp/stats/cuped.py:300`
...
### M4. O'Brien-Fleming and Pocock boundaries are correct but approximate — `openxp/stats/sequential.py:340-346`
...
`discovery.py:20-38` defines `TREATMENT_COLUMN_HINTS`, `CONTROL_VALUE_HINTS`, `ID_COLUMN_PATTERNS`, `TIMESTAMP_NAME_PATTERNS` as module-level tuples and uses them exclusively via iteration. Grep across `openxp/data/` for literal `'variant'`, `'conversion'`, `'revenue'` returns only the hints tuple and test fixtures. Clean.
...
### M7. Atomic writes are actually atomic — `openxp/storage/store.py:59-82`
...
### S1. `bayesian.py` uses `pytest`-like blank lines between top-level items inconsistently compared to `ab_tests.py` — `openxp/stats/bayesian.py` throughout
...
### S2. `sequential.py` uses mixed error-return shapes — `openxp/stats/sequential.py:145-153, 239-245`
...
### S3. `cuped.py:331` string-truthy check on `raw_var` — `openxp/stats/cuped.py:331`
```python
...
### S4. `snowflake_loader.py` `where` parameter is a raw-SQL injection vector — `openxp/data/snowflake_loader.py:270-272, 289`
...
### S6. `openxp/stats/__init__.py` does not export `_decide` or `_relative_lift_samples` — **this is correct**; style positive.
...
### T6. Storage test `test_store_from_env_default_not_touched` explicitly does NOT build the store to avoid mkdir-ing `~/.openxp` — good, this is exactly the right pattern. Positive.
...
### I1. Wave 1 adds 3 new modules to `openxp.stats` that agents/skills must know about, but skill.md's "Stats Function Quick Reference" table is pre-Wave-1 — `skill.md:151-158`
...
### I2. `MetricDefinition.type` values (`proportion`, `mean`, `ratio`) don't cover the Bayesian or CUPED test functions — `openxp/metrics/schema.py:17-19, 150-163`
...
### I4. `openxp/data/__init__.py` imports `DuckDBLoader` at module top — `openxp/data/__init__.py:24`
`duckdb_loader.py` itself lazy-imports duckdb, but `from openxp.data import ...` executes the class body at import time. This is fine — the `_import_duckdb()` call is inside `__init__`, not at module level. Verified clean.

### I5. No name collisions across the five new modules. Verified with `import openxp; import openxp.stats, openxp.storage, openxp.metrics, openxp.data` — no shadowing.
...
- **`openxp/data/base.py:39-53` `SchemaDiscovery.to_dict`** has no direct test (only exercised transitively).
- **`openxp/data/base.py:85-97` `LoadResult.to_dict`** — same.
- **`openxp/data/csv_loader.py:136-141` `CSVLoader.stream` error paths** — stream is tested for the happy path but not for file-not-found or empty file.
- **`openxp/stats/cuped.py:114-116` variance-zero fallback** (both pre and post) — no test.
- **`openxp/stats/sequential.py:82-116` `_msprt_core` with extreme n_eff** (n=2, the minimum allowed) — no direct assertion.
- **`openxp/stats/bayesian.py:125-143` `_decide`** is covered via the public API tests but has no unit test with edge cases on the thresholds (e.g., loss_ship_t_rel exactly equal to threshold_ship).
- **`openxp/storage/store.py:85-100` `_append_jsonl` under concurrent writers** — no test. Low priority (single-process assumption is stated).
- **`openxp/storage/store.py:444-458` `delete_experiment`** is tested, but the test does not confirm that the JSONL log is also removed along with the directory. It is (via `shutil.rmtree`) but an explicit assertion would be nice.
- **`openxp/metrics/registry.py:19-27` `_default_metrics_dir` fallback to `~/.openxp/metrics`** — no test (would require monkeypatching HOME).
...
1. **[BLOCKER]** Fix `skill.md` and `MODES.md` to remove references to non-existent functions: `power_ratio`, `extension_estimate`, `fishers_exact_test`, `guardrail_test`, `denominator_srm`, `cohens_h`, `openxp.stats.prep.prepare_experiment_data`. Either implement them in Wave 2 or rewrite the affected mode steps to use only what `openxp.stats` actually exports today.
2. **[BLOCKER]** Remove the `computation_trace` contract from `skill.md:149` and `MODES.md:196, 222, 260`, or implement it in every `openxp.stats` function. Current state is a silent integration landmine.
...
8. **[MEDIUM]** Update top-level `openxp/__init__.py` docstring to include Bayesian/CUPED/sequential usage examples OR stop advertising `from openxp.stats import ...` patterns at the root docstring and let `openxp.stats.__init__` own that docs surface.
